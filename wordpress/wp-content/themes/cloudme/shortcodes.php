...
function homeimage_func($atts, $content = null){
    ...
    <div class="page-top page-880" style="<?php if($bg) { echo 'background-image:url('.esc_url($bg2).');'; } if($mheight) { echo 'min-height: '.esc_attr($mheight).';'; } ?>">
      <div class="message container">
            ...
            <?php } if($btn) { ?><a href="<?php echo esc_url($link); ?>" class="small radius btn"><?php echo htmlspecialchars_decode($btn); ?></a><?php } ?>
      </div>
    </div>
   ...
   $tabs_panel[] = sprintf(
            '<div class="item">
                <div class="slidercaption">
                    <h3>%s</h3>
                    <p>%s</p>
                    <div class="viewplans hidden-sm"><a class="btn btn-slide" href="%s">%s</a></div>
                </div>
                %s
            </div>',
    $slide['btext'],
...
function bgvideo_func($atts, $content = null){
    ...
    <div id="video-container" class="page-top">
      ...
      <video id='video-player' autoplay loop<?php if($mute) { ?> muted<?php } ?>>
          ...
          <source src="<?php echo esc_url($mp4); ?>" type="video/webm">
          <source src="<?php echo esc_url($mp4); ?>" type="video/ogg">
      </video>
              ...
              <div class="row">
                <div class="col-md-10 col-md-offset-1">
                  ...
                  <div class="text-center" id="message"><img src="<?php echo esc_url($img); ?>" alt="">
                    ...
                    <?php if($bridge) { ?>
                    <form method="post" name="whmcs_com" id="whmcs" action="<?php echo esc_url($actionlink); ?>/?ccce=cart&amp;a=add&amp;domain=register">
                    <?php }else{ ?>
                    <form method="post" name="whmcs_com" id="whmcs" action="<?php echo esc_url($actionlink); ?>/cart.php?a=add&amp;domain=register">
                    <?php } ?>
...
function ggmap2_func($atts, $content = null){
   ...
   if(!$zoom){
    ...
    // Multiple Markers
    var markers = [
        ['Data Space, Ohio, United States', <?php echo esc_js($latlong1); ?>],
        ['CIX-IT, Frankfurt, Germany', <?php echo esc_js($latlong2); ?>],
        ['AllExpand, Singapore, Asia-Pacific', <?php echo esc_js($latlong3); ?>]
    ];

    // Info Window Content
    var infoWindowContent = [
        ['<div class="info_content"><?php echo esc_js($info1); ?></div>'],
        ['<div class="info_content"><?php echo esc_js($info2); ?></div>'],
        ['<div class="info_content"><?php echo esc_js($info3); ?></div>']
    ];
