...
define("CC_WHMCS_BRIDGE_VERSION","3.7.4");
...
function cc_whmcs_bridge_output($page=null) {
    ...
    if (strstr($http, 'domainchecker.php') !== false && isset($_REQUEST['a'], $_REQUEST['domain'])) {
        $http = str_replace('domainchecker.php', 'cart.php', $http);
        cc_whmcs_log(0, '[URL '.$ref.'] URL Adjusted to cart.php');
    }
    ...
    if (!$news->curlInstalled()) {
        ...
        if ($cc_whmcs_bridge_to_include=='verifyimage' || (isset($_REQUEST['showqrimage']) && $_REQUEST['showqrimage'] == 1)
            ...
            if (!$news->redirect) {
                ...
                } else if (isset($_REQUEST['check'], $_REQUEST['addtocart'], $_REQUEST['domain']) || isset($_REQUEST['responseType']) && $_REQUEST['responseType'] == 'json' || isset($_REQUEST['a'], $_REQUEST['domain'])) {
